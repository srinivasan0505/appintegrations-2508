...
func GetdataintegrationHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		args, ok := request.Params.Arguments.(map[string]any)
		if !ok {
			return mcp.NewToolResultError("Invalid arguments object"), nil
		}
		IdentifierVal, ok := args["Identifier"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: Identifier"), nil
		}
		Identifier, ok := IdentifierVal.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: Identifier"), nil
		}
		url := fmt.Sprintf("%s/dataIntegrations/%s", cfg.BaseURL, Identifier)
		...
		var result models.GetDataIntegrationResponse
...
func CreateGetdataintegrationTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_dataIntegrations_Identifier",
		mcp.WithDescription("<p>Returns information about the DataIntegration.</p> <note> <p>You cannot create a DataIntegration association for a DataIntegration that has been previously associated. Use a different DataIntegration, or recreate the DataIntegration using the <a href="https://docs.aws.amazon.com/appintegrations/latest/APIReference/API_CreateDataIntegration.html">CreateDataIntegration</a> API.</p> </note>"),
		...
		mcp.WithString("Identifier", mcp.Required(), mcp.Description("A unique identifier.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    GetdataintegrationHandler(cfg),
	}
}
