...
func ListeventintegrationassociationsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/eventIntegrations/%s/associations%s", cfg.BaseURL, Name, queryString)
		...
		var result models.ListEventIntegrationAssociationsResponse
...
func CreateListeventintegrationassociationsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_eventIntegrations_Name_associations",
		mcp.WithDescription("Returns a paginated list of event integration associations in the account. "),
		...
		mcp.WithString("Name", mcp.Required(), mcp.Description("The name of the event integration. ")),
		mcp.WithString("nextToken", mcp.Description("The token for the next set of results. Use the value returned in the previous response in the next request to retrieve the next set of results.")),
		mcp.WithNumber("maxResults", mcp.Description("The maximum number of results to return per page.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    ListeventintegrationassociationsHandler(cfg),
	}
}
